...
static glm::vec3 calculateFaceNormal(const glm::vec3& v0, const glm::vec3& v1,
                                     const glm::vec3& v2) {
...
bool Vocksel::Model::loadFromFile(std::string path) {
  ...
  while (std::getline(file, line)) {
    ...
    if (tokens[0] == "v") {  // Vertex positions
      positions.emplace_back(std::stof(tokens[1]), std::stof(tokens[2]),
                             std::stof(tokens[3]));
      ...
      normals.emplace_back(std::stof(tokens[1]), std::stof(tokens[2]),
                           std::stof(tokens[3]));
    } else if (tokens[0] == "f") {
      ...
      if (vertex_count == 3) {  // Triangle
        ...
        std::cerr << "Unsupported face with " << vertex_count << " vertices"
                  << std::endl;
      }
    }
  }
  ...
  for (size_t i = 0; i < position_indices.size(); i++) {
    ...
    if (i < tex_coord_indices.size())
      key += std::to_string(tex_coord_indices[i]) + "_";
    ...
    if (normals.empty() && i + 2 < position_indices.size()) {
      ...
      if (idx0 < positions.size() && idx1 < positions.size() &&
          idx2 < positions.size()) {
        faceNormal = calculateFaceNormal(positions[idx0], positions[idx1],
                                         positions[idx2]);
      }
    }
  ...
  // Create the mesh (8 floats per vertex: position(3) + texcoord(2) +
  // normal(3))
  addMesh(std::make_unique<StaticMesh>(
      vertices.data(), static_cast<int>(vertices.size()), indices.data(),
      static_cast<int>(indices.size()), 8));
...
void Vocksel::Model::cleanUp() { meshes_.clear(); }

Vocksel::Model::~Model() { cleanUp(); }
