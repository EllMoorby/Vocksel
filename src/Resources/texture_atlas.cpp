...
void Vocksel::TextureAtlas::loadFromFolder(const std::string& path) {
  ...
  std::cout << "Creating atlas size: " << atlas_size_ << "x" << atlas_size_
            << std::endl;
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, atlas_size_, atlas_size_, 0, GL_RGBA,
...
GLuint Vocksel::TextureAtlas::getAtlasTexture() const { return atlas_texture_; }

float Vocksel::TextureAtlas::getTileScale() const {
  return 1.0f /
         (static_cast<float>(atlas_size_) / static_cast<float>(tile_size_));
}

glm::vec2 Vocksel::TextureAtlas::getUVOffset(
...
Vocksel::TextureAtlas::~TextureAtlas() { cleanUp(); }
