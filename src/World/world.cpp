...
void Vocksel::World::init() {
  ...
  setGenerationParams(0.001f, 5, 2.0f, 0.5f);
...
void Vocksel::World::initDebug() {
  EngineServices::debugGUI().addPanel("World", [this]() {
    ...
    static auto load_world_at = glm::ivec3(0);
...
void Vocksel::World::updateChunksAroundPosition(glm::vec3 position) {
  ...
  auto middle_chunk = glm::ivec3(chunk_pos_f);
    ...
    for (int y = -height; y <= height; y++) {
      for (int x = -range; x <= range; x++) {
        ...
        if (!chunk_map_.contains(chunk_pos)) {
          auto chunk =
              std::make_unique<Chunk>(chunk_pos * int(Constants::CHUNK_SIZE));
...
const glm::vec3 &Vocksel::World::getSpawnPosition() { return spawn_position_; }

void Vocksel::World::setSpawnPosition(glm::vec3 position) {
  spawn_position_ = position;
}

void Vocksel::World::setGenerationParams(float freq, int octaves,
...
void Vocksel::World::update(glm::vec3 position) {
  ...
  int num_processed = 0;
  while (num_processed < Constants::CHUNKS_PER_FRAME &&
         !dirty_chunks_.empty()) {
...
Vocksel::World::~World() { MarchingCubes::cleanUp(); }
