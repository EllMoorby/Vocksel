...
void Vocksel::MarchingCubes::createEdgeTableTexture() {
  ...
  glTexImage1D(GL_TEXTURE_1D, 0, GL_R32UI, 256, 0, GL_RED_INTEGER,
               GL_UNSIGNED_INT, edge_table);
...
void Vocksel::MarchingCubes::createTriTableTexture() {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_R32I, 16, 256, 0, GL_RED_INTEGER, GL_INT,
               tri_table);
...
void Vocksel::MarchingCubes::computeMesh(const Texture3D &density_tex,
                                         ComputeMesh &output) {
#if DEBUG
  ...
  glBufferSubData(GL_DRAW_INDIRECT_BUFFER, 0,
                  sizeof(DrawElementsIndirectCommand), &cmd);

  glActiveTexture(GL_TEXTURE0);
  glBindTexture(GL_TEXTURE_3D, density_tex.getId());

  glBindImageTexture(0, density_tex.getId(), 0, GL_TRUE, 0, GL_READ_ONLY,
                     GL_R32F);
  ...
  compute_shader_.setFloat("voxel_size", float(Constants::CHUNK_SIZE) /
                                             float(Constants::CUBES_PER_CHUNK));
...
Vocksel::MarchingCubes::~MarchingCubes() = default;
