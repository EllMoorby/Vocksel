...
Vocksel::DensityField::DensityField(glm::ivec3 size, uint32_t gpu_format)
    : size_(size), texture_(), voxels_(size.x * size.y * size.z, 0.f) {}
...
void Vocksel::DensityField::uploadToGPU() {
  ...
  glTexSubImage3D(GL_TEXTURE_3D, 0, 0, 0, 0, texture_.getSize().x,
                  texture_.getSize().y, texture_.getSize().z, GL_RED, GL_FLOAT,
                  voxels_.data());
