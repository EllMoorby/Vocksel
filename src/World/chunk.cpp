...
Vocksel::Chunk::Chunk(glm::vec3 position)
    : position_(position),
      density_field_(glm::ivec3(Constants::CUBES_PER_CHUNK + 1), GL_R32F) {}
...
void Vocksel::Chunk::generateTerrain(FastNoiseLite &noise) {
  ...
  const float voxel_size =
      float(Constants::CHUNK_SIZE) / float(Constants::CUBES_PER_CHUNK);
  ...
  for (int z = 0; z <= Constants::CUBES_PER_CHUNK; z++) {
    for (int x = 0; x <= Constants::CUBES_PER_CHUNK; x++) {
      ...
      float base_height =
          noise.GetNoise(world.x * 10.f, world.z * 10.f) * 32.f - 16.f;
      ...
      for (int y = 0; y <= Constants::CUBES_PER_CHUNK; y++) {
        ...
        float density = std::tanh(raw_density *
                                  0.1f);  // Adjust 0.1f for sharper transition
