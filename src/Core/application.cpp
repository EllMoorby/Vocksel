...
void Vocksel::Application::initWindow() {
  ...
  // Create window
  window_ = glfwCreateWindow(Constants::SCREEN_WIDTH, Constants::SCREEN_HEIGHT,
                             "Vocksel", nullptr, nullptr);
  if (!window_) {
...
// https://learnopengl.com/In-Practice/Debugging
void APIENTRY Vocksel::Application::glDebugOutput(
...
void Vocksel::Application::run() {
  glClearColor(game_.getClearColor().r, game_.getClearColor().g,
               game_.getClearColor().b, game_.getClearColor().a);
  ...
  while (!glfwWindowShouldClose(window_)) {
    ...
    if (second_count >= 1.0f) {
      glfwSetWindowTitle(
          window_, std::format("Vocksel | {}", 1.0f / delta_time).c_str());
      second_count = 0;
    }
...
void Vocksel::Application::framebufferSizeCallback(GLFWwindow *window,
...
void Vocksel::Application::mouseCallback(GLFWwindow *window_, double xpos,
                                         double ypos) {
