...
void Vocksel::Game::debugGUI() {
    EngineServices::debugGUI().addPanel("Game", [this]() {
        ...
        std::vector<std::string> model_names = EngineServices::models().getModelNames();
        ...
        std::vector<std::string> texture_names = EngineServices::resources().getTextureNames();
        ...
        static glm::vec3 model_position = glm::vec3(0.0f);
        ImGui::InputFloat3("Position", glm::value_ptr(model_position));

        static glm::vec3 model_scale = glm::vec3(0.0f);
        ImGui::InputFloat3("Scale", glm::value_ptr(model_scale));


        if (ImGui::Button("Spawn Model")) {
            model_instances_.push_back(
                ModelInstance::create(model_position, model_names[model_index].c_str(), texture_names[texture_index].c_str(), model_scale)
            );
        }
    });
