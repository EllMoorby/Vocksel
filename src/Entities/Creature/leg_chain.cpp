...
Vocksel::LegChain::LegChain(glm::vec3 root, glm::vec3 forward, glm::vec3 offset)
    : root_(root),
      forward_(forward),
      offset_(offset),
...
void Vocksel::LegChain::updateLegShape(float delta_time) {
  ...
  for (auto& segment : segments_) {
    auto new_pos = glm::vec3(
        root_.x, root_.y + total_length + segment.getLength(), root_.z);
  ...
  for (int x = 0; x < 20; x++) {
    uint32_t idx = 0;
    for (auto& segment : segments_) {
      glm::vec3 world_vel = forward_ * velocities_[idx].x +
                            Constants::WORLD_UP * velocities_[idx].y +
                            right_ * velocities_[idx].z;
      segment.setTipPosition(segment.getTipPosition() +
                             (world_vel * delta_time * 100.f));
      idx++;
    }
    ...
    for (size_t i = segments_.size() - 1; i > 0; i--) {
      glm::vec3 dir =
          segments_[i - 1].getTipPosition() - segments_[i].getTipPosition();
      if (glm::length(dir) > 0.0001f) {
        dir = glm::normalize(dir);
        segments_[i - 1].setTipPosition(segments_[i].getTipPosition() +
                                        dir * segments_[i - 1].getLength());
      }
    }
    ...
    for (auto& segment : segments_) {
      glm::vec3 dir = segment.getTipPosition() - current_base;
      if (glm::length(dir) > 0.0001f) {
        dir = glm::normalize(dir);
        segment.setTipPosition(current_base + dir * segment.getLength());
        current_base = segment.getTipPosition();
      }
    }

    if (glm::distance(segments_.back().getTipPosition(), target_) <
        tolerance_) {
      break;
    }
  }
...
void Vocksel::LegChain::update(float delta_time, glm::vec3 root,
                               glm::vec3 forward) {
  root_ = root;
  forward_ = forward;
  updateVectors();
  ...
  if (!segments_.empty()) {
    auto horizontal_distance =
        glm::length(segments_.back().getTipPosition() - rest_position_);
...
void Vocksel::LegChain::updateVectors() {
  ...
  rest_position_ = root_ + (right_ * offset_.x) + (forward_ * offset_.z) +
                   (Constants::WORLD_UP * offset_.y);
}

void Vocksel::LegChain::setVelocity(glm::vec3 velocity, uint32_t idx) {
  if (idx + 1 > segments_.size() || idx + 1 > velocities_.size()) {
    std::cerr << "Index out of bounds when setting velocity for leg joint"
              << std::endl;
  }
