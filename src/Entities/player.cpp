...
void Vocksel::Player::initDebug() {
  EngineServices::debugGUI().addPanel("Player", [this]() {
    ImGui::Text("Position: (%.2f, %.2f, %.2f)", position_.x, position_.y,
                position_.z);
    ImGui::Text("Velocity: (%.2f, %.2f, %.2f)", velocity_.x, velocity_.y,
                velocity_.z);
    ImGui::Text("Acceleration: (%.2f, %.2f, %.2f)", acceleration_.x,
                acceleration_.y, acceleration_.z);
    ImGui::Separator();
    ImGui::Text("Pitch: %.2f || Yaw: %.2f", pitch_, yaw_);
    ImGui::Text("Camera Offset: (%.2f, %.2f, %.2f)", camera_offset_.x,
                camera_offset_.y, camera_offset_.z);
...
void Vocksel::Player::applyPhysics(float deltaTime) {
  ...
  auto horizontal_vel = glm::vec3(velocity_.x, 0.0f, velocity_.z);
  auto vertical_vel = glm::vec3(0.0f, velocity_.y, 0.0f);
...
bool Vocksel::Player::checkCollision(float x, float y, float z,
                                     const glm::vec3& size) {
  return false;
}
